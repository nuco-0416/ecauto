...
1. **括弧なしの`.co.jp限定`パターン**
   - 例: `【.co.JP限定 純正品】` → `【純正品】`として残すべきところ、`.co.JP限定`部分が残っていた
...
#### 1. ng_keywords.jsonへのパターン追加

```json
{
  "keywords": [
    ".co.jp限定品",   // 追加: 「限定品」パターン
    ".co.jp限定の",   // 追加: 「限定の」パターン
    ".co.jp限定",     // 追加: 括弧なしパターン
    ".co.jp 限定",    // 追加: 括弧なし・スペースありパターン
    // ... 既存のキーワード
  ]
}
```

#### 2. _cleanup_residue()メソッドの実装

`common/ng_keyword_filter.py`に残骸クリーンアップ処理を追加:
...
| 入力 | 出力 | 結果 |
|-----|------|------|
| `【.co.jp限定品】マヌカヘルス...` | `マヌカヘルス...` | ✅ |
| `ムシューダ [.co.jp限定] 衣類用...` | `ムシューダ 衣類用...` | ✅ |
| `■ .co.jp限定のMKC.MXJ×3個...` | `■ MKC.MXJ×3個...` | ✅ |
...
- 追加対策として、残骸クリーンアップ処理（`_cleanup_residue()`）を実装し、空括弧や助詞のみの残骸を自動削除するようにした
