"""
マルチアカウント対応アップロードデーモン（並列処理版）

アカウント別に独立したプロセスとして起動
multi_account_manager.pyから呼び出される

新機能:
- アカウント指定によるキューフィルタリング
- アカウント別ログファイル
- 複数プロセスによる並列アップロード
"""
...
class UploadSchedulerAccountDaemon(DaemonBase):
    """
    アカウント別アップロードスケジューラー（並列処理版）

    特徴:
    - アカウント指定によるキューフィルタリング
    - プラットフォーム + アカウント別に独立して起動可能
    - DaemonBase継承による堅牢性
    - Chatwork通知統合
    - エラーリトライ機能
    - 構造化ログ
    ...
    def __init__(
        ...
        # DaemonBase初期化（アカウント別ログ）
        super().__init__(
            name=f'upload_scheduler_{platform}_{account_id}',  # アカウント別ログ
        ...
        self.logger.info(f"アカウント: {account_id}")
    ...
    def execute_task(self) -> bool:
        ...
        try:
            ...
            items = self.queue_manager.get_scheduled_items_due(
                ...
                platform=self.platform,
                account_id=self.account_id  # アカウントフィルタを追加
            )
            ...
            if failed_count > 0 and failed_count > success_count:
                if self.notifier:
                    self.notifier.notify(
                        ...
                        title=f"[WARNING] {self.platform.upper()} / {self.account_id} アップロード失敗率が高い",
                        message=(
    ...
    def _send_completion_notification(
        ...
        self.send_completion_report(
            task_name=f'{self.platform.upper()} / {self.account_id} 商品出品',
            stats=report_stats
        )
    ...
    def _upload_single_item(self, queue_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['status'] == 'success':
                ...
                # デバッグ: 画像データの確認
                self.logger.info(f"[DEBUG] item_data keys: {list(item_data.keys())}")
                self.logger.info(f"[DEBUG] item_data['images'] type: {type(item_data.get('images'))}")
                self.logger.info(f"[DEBUG] item_data['images'] length: {len(item_data.get('images', []))}")

                # 画像アップロード
                images = item_data.get('images')
                self.logger.info(f"[DEBUG] images check: type={type(images)}, bool={bool(images)}, len={len(images) if images else 0}")

                if images:
                    self.logger.info(f"画像アップロード開始: {len(images)}枚")
                    ...
                    self.logger.info(f"[DEBUG] 画像アップロード処理完了後")
                else:
                    self.logger.warning(f"[DEBUG] item_data['images']が空のため、画像アップロードをスキップしました")

                self.logger.info(f"[DEBUG] if/else ブロック抜けた")
                # ステータスを「成功」に更新
                self.logger.info(f"[DEBUG] キューステータス更新開始: queue_id={queue_id}")
                self.queue_manager.update_queue_status(
                    queue_id=queue_id,
                    status='success',
                    result_data={'platform_item_id': platform_item_id}
                )
                self.logger.info(f"[DEBUG] キューステータス更新完了: queue_id={queue_id}")

                self.logger.info(f"[DEBUG] _upload_single_item return前")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='マルチアカウント対応アップロードスケジューラー（並列処理版）'
    )
    parser.add_argument(
        '--platform',
        required=True,
