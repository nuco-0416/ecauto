...
def main():
    ...
    if args.now:
        ...
        print("新しいスケジュール: 変更なし（--nowオプションを指定してください）")
        return
    ...
    if len(items) > 0:
        ...
        for i in range(min(3, len(items))):
            item = items[i]
            new_time = base_time + timedelta(seconds=args.interval * i)
    ...
    with db.get_connection() as conn:
        ...
        for i, item in enumerate(items):
            queue_id = item[0]
            new_scheduled_at = base_time + timedelta(seconds=args.interval * i)
    ...
    if len(items) > 0:
        end_time = base_time + timedelta(seconds=args.interval * (len(items) - 1))
